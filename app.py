...
# [성식님 제안 로직] 강력한 스크롤 초기화 함수 (components.html 사용)
...
def navigate_to(view, target=None):
    ...
    st.session_state.force_scroll = True # 스크롤 트리거 활성화
    st.rerun()
...
<style>
    ...
    .block-container {{ padding-top: 4.5rem !important; padding-bottom: 4rem !important; max-width: 100% !important; }}
...
# 4. 화면 전환 및 클릭 시 상단 고정 (성식님 제안 로직 트리거)
...
with app_canvas:
    ...
    # [2] CHEER VIEW
        ...
        if db:
            ...
            if "post_id" in query_params:
                target_post = next((c for c in cheers if c["id"] == query_params["post_id"]), None)
                if target_post: st.session_state.modal_post = target_post
                st.query_params.clear()
            ...
            if not cheers:
                ...
                components.html(
                    f"""<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>body {{ margin:0; padding:0; background:transparent; }} .grid {{ display:grid; grid-template-columns:repeat(3, 1fr); gap:8px; padding:5px; }} .gallery-item {{ display:block; position:relative; aspect-ratio:1/1; border-radius:10px; overflow:hidden; background:#F2F2F7; box-shadow: 0 1px 4px rgba(0,0,0,0.1); }} .gallery-item img {{ width:100%; height:100%; object-fit:cover; }} .name-tag {{ position:absolute; bottom:0; width:100%; background:rgba(0,0,0,0.4); color:white; font-size:9px; text-align:center; padding:2px 0; overflow:hidden; }}</style></head><body><div class="grid">{gallery_items}</div></body></html>""",
