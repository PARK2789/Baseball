...
# 2. 강력한 스크롤 초기화 함수 (성식님 제안 로직 100% 반영)
...
current_post_id = query_params.get("post_id")

# 파라미터가 있으면 뷰를 즉시 'cheer'로 고정 (메인 화면 플래시 방지)
if current_post_id:
...
# 5. 메인 앱 컨테이너 (이 안에서만 화면을 전환하여 플래시 현상 제거)
...
with main_app:
    ...
    # [2] CHEER FEED VIEW (갤러리 및 이벤트)
        ...
        if db:
            ...
            # 사진 클릭 상세 보기 처리
            if current_post_id:
                target_post = next((c for c in cheers if c["id"] == current_post_id), None)
                if target_post:
    ...
    # [4] UPLOAD / [5] EVENT / [6] DETAIL
    elif st.session_state.view in ['upload', 'event_upload', 'detail']:
