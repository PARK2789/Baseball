...
<style>
    ...
    .hero-section {{
        background: linear-gradient(rgba(0,0,0,0.1), rgba(0,0,0,0.4)), url('data:image/jpeg;base64,{base64.b64encode(open("stadium.jpg", "rb").read()).decode() if os.path.exists("stadium.jpg") else ""}');
...
def show_post_modal(post):
    ...
    if is_admin:
        if st.button("🗑️ 관리자 삭제", key="modal_admin_del"):
...
with app_canvas:
    ...
    if st.session_state.view == 'home':
        ...
        for name, info in program_data.items():
            st.markdown(f'<div class="program-card" style="background-image: url(\'data:image/jpeg;base64,{base64.b64encode(open(info.get("bg_file", ""), "rb").read()).decode() if os.path.exists(info.get("bg_file", "")) else ""}\');"><div style="position:relative; z-index:2; text-shadow: 0px 2px 4px rgba(0,0,0,0.5);"><div style="font-size:11px; font-weight:800; color:white; background:rgba(0,0,0,0.4); display:inline-block; padding:2px 8px; border-radius:4px; margin-bottom:4px;">{info.get("tag")}</div><div style="font-size: 20px; font-weight: 800; color:white;">{name}</div></div></div>', unsafe_allow_html=True)
    ...
    # [2] CHEER FEED (갤러리)
        ...
        if db:
            ...
            if not cheers: st.info("아직 사진이 없습니다.")
            else:
                # [수정] 3개씩 묶어서 컬럼 생성 (CSS가 3열로 강제 고정함)
                for i in range(0, len(cheers), 3):
                    ...
                    for idx, post in enumerate(row):
                        with cols[idx]:
                            ...
                            """, unsafe_allow_html=True)
                            # 2. 이미지 위에 덮어씌워질 투명 버튼
                            if st.button("", key=f"btn_{post['id']}"):
