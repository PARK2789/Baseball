...
def compress_image(uploaded_file):
    ...
    img = ImageOps.exif_transpose(img) # 사진 회전 보정
...
<style>
    ...
    .stButton>button {{ 
        ...
        border: none; box-shadow: 0 4px 10px rgba(0,0,0,0.08);
    }}
    ...
    /* 사진 클릭 기능을 위한 투명 버튼 오버레이 */
    div[data-testid="column"] {{
        position: relative;
    }}
    .img-click-btn button {{
        position: absolute !important; top: 0 !important; left: 0 !important;
        width: 100% !important; height: 100% !important;
        background: transparent !important; color: transparent !important;
        border: none !important; z-index: 10 !important;
    }}
    
    .gallery-img-box {{
        width: 100%; padding-top: 100%; position: relative;
        border-radius: 12px; overflow: hidden; background-color: #F2F2F7;
...
def show_post_modal(post):
    ...
    st.write(post['text'])
    st.caption(f"작성일시: {post.get('timestamp', datetime.now()).strftime('%H:%M')}")
    if is_admin:
        if st.button("🗑️ 관리자 삭제", key="modal_del"):
...
with app_canvas:
    ...
    # [2] CHEER FEED VIEW (갤러리)
        ...
        if db:
            ...
            if not cheers:
                ...
                rows = [cheers[i:i + 3] for i in range(0, len(cheers), 3)]
                for row in rows:
                    cols = st.columns(3)
                    for idx, post in enumerate(row):
                        with cols[idx]:
                            ...
                            """, unsafe_allow_html=True)
                            # 투명 버튼을 이미지 위에 덮어씌움 (image_btn 텍스트 삭제)
                            st.markdown('<div class="img-click-btn">', unsafe_allow_html=True)
                            if st.button("", key=f"click_{post['id']}", help="크게 보기"):
    ...
    # [4] UPLOAD VIEW
        ...
        <div class="example-box">
            ...
            <div style="font-size:14px; color:#3A3A3C; line-height:1.6;">
                ...
                <b>(예시: CEO와 셀카, 응원 장면, LG 득점 순간, 인증샷 등)</b><br><br>
    ...
    # [5] EVENT UPLOAD VIEW
        ...
        st.markdown('<h2 style="font-weight:900; text-align:center; margin-bottom:5px;">🎯 이벤트 참여</h2>', unsafe_allow_html=True)
        st.markdown(f"""<div class="example-box"><div style="font-weight:800; color:#FF3B30; font-size:15px; margin-bottom:8px;">⚾️ 오늘의 주인공은 누구?</div><div style="font-size:14px; color:#3A3A3C;">오늘 첫 홈런과 첫 안타의 주인공을 맞춰보세요!<br>정답자에게는 특별한 선물을 드립니다.</div></div>""", unsafe_allow_html=True)
        with st.container():
